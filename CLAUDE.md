...
- Les gardes ci-dessus ne voient pas les liens physiques : un lien dur est un nom de plus sur les mêmes données, indiscernable d'un fichier régulier (`symlink_metadata` comme `canonicalize` le résolvent sous le profil). Un lien dur posé sous le profil vers des données situées ailleurs sera donc supprimé — limite acceptée, faute de contre-mesure sans coût.
