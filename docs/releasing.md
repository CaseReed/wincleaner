...
The GitHub Release body is generated by `scripts/release-notes.mjs`: the raw
markdown `## [x.y.z]` section of `CHANGELOG.md` for the tagged version (as
written, unlike the plain-text conversion `extract-whats-new.mjs` embeds in
the app), followed by the unsigned-installers note and a link to the full
